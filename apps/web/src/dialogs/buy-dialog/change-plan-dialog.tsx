...
export const ChangePlanDialog = DialogManager.register(
  function ChangePlanDialog(props: ChangePlanDialogProps) {
    ...
    const currentPlan = plans.find((p) => p.id === subscription?.productId);
    if (!subscription || !currentPlan) return null;
    return (
      <BaseDialog
        title={`Change plan`}
        description={`You are currently on ${
          PLAN_METADATA[currentPlan.plan].title +
          ` (${PERIOD_METADATA[currentPlan.period].title})`
        } plan.`}
        isOpen={true}
        onClose={onClose}
        positiveButton={{
          ...
          {changeSummary.length > 0 ? (
            <Flex sx={{ flexDirection: "column", gap: 1, mt: 1 }}>
              ...
              <Text variant="subBody">
                Note: These are estimated amounts. The real amounts could differ
                slightly.
              </Text>
            </Flex>
          ) : null}
