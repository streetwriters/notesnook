...
const navigationHistory = new Map<"home" | "notebooks" | "tags", string>();
...
const tabs = [
  {
    ...
    path: "/",
    selectedPaths: routesInit.map((r) => r.path).concat(["/colors"])
  },
  {
    id: "notebooks",
    icon: NotebookIcon,
  ...
  { id: "tags", icon: TagIcon, title: strings.routes.Tags(), path: "/tags" }
];
...
function NavigationMenu(props: NavigationMenuProps) {
  ...
  const _navigate = useCallback(
    (path: string) => {
      ...
      const previousRoute = findPreviousRoute(path);
      navigate(
        !previousRoute || previousRoute === location ? path : previousRoute
      );
      // navigate(path);
    },
  ...
  useEffect(() => {
    if (location.startsWith("/notebooks")) {
      navigationHistory.set("notebooks", location);
    } else if (location.startsWith("/tags")) {
      navigationHistory.set("tags", location);
    } else {
      navigationHistory.set("home", location);
    }
        ...
        >
          <svg
            style={{
              ...
              // width: "",
              // flexBasis: 40,
              width: isTablet ? 20 : isMobile ? 30 : 40,
              height: isTablet ? 20 : isMobile ? 30 : 40
              // alignSelf: "start"
            }}
          >
            <use href="#full-logo" />
      ...
      >
        {tabs.map((tab) => (
          <NavigationItem
            ...
            selected={
              tab.selectedPaths
                ? !!tab.selectedPaths.find((p) => location.includes(p))
                : location.includes(tab.path)
            }
            onClick={() => _navigate(tab.path)}
            showTitle={false}
          />
        ))}
      </Flex>
...
function NavigationDropdown() {
  ...
  return (
    <Flex sx={{ flexDirection: "row", alignItems: "center", gap: 1 }}>
      ...
      >
        {theme === "dark" ? <LightMode size={16} /> : <DarkMode size={16} />}
      </Button> */}
      <Flex
        onClick={(e) => {
          e.preventDefault();
          Menu.openMenu(
            [
              {
                type: "popup",
                component: ({ onClick }) => <UserProfile minimal />,
                key: "profile"
...
function findPreviousRoute(location: string) {
  console.log("navigation history", location, navigationHistory);
  const level = location.split("/").length;
  if (level > 2) {
    return location;
  }

  if (location.startsWith("/notebooks")) {
    return navigationHistory.get("notebooks");
  }
  if (location.startsWith("/tags")) {
    return navigationHistory.get("tags");
  }
  if (location === "/") {
    return navigationHistory.get("home");
  }
  return undefined;

  let level1 = location.split("/").length;
  ...
  for (let i = history.length - 1; i >= 0; --i) {
    ...
    if (location === "/") {
      if (!route.startsWith("/notebooks") && !route.startsWith("/tags")) {
        nestedRoute = route;
        // nestedRoute = history
        // .reverse()
        // .find((r) => !r.startsWith("/notebooks") && !r.startsWith("/tags"));
      }
      break;
    }
