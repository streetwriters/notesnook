...
async function moveNotesToTrash(ids: string[], confirm = true) {
  ...
  if (ids.length === 1) {
    const noteId = ids[0];
    if (
      lockedIds.includes(noteId) &&
      !(await Vault.unlockNote(noteId, "unlock_and_delete_note"))
    )
      return;
    lockedIds.splice(lockedIds.indexOf(noteId), 1);
  }

  const items = ids.filter(
    (id) => !lockedIds.includes(id) && !db.monographs.isPublished(id)
  );
