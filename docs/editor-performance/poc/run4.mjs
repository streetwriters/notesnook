...
const PORT = 8794, CDP_PORT = 9336;

const server = http.createServer((req, res) => {
  const f = path.join(DIR, req.url === "/" ? "proof4.html" : req.url.split("?")[0]);
...
const userDir = path.join(DIR, ".chrome-profile4");
const chrome = spawn("/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", [
  ...
  `http://localhost:${PORT}/proof4.html`
...
for (let i = 0; i < 100; i++) { if (await ev("window.__ready4===true").catch(() => false)) break; await new Promise((r) => setTimeout(r, 200)); }

const out = {};
out.cvTest = JSON.parse(await ev("JSON.stringify(window.cvTest([500,1500,3000]))"));
out.cvEditingIntegrity = JSON.parse(await ev("JSON.stringify(window.cvEditingIntegrity(1500))"));
