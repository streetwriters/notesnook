...
const PORT = 8795, CDP_PORT = 9337;

const server = http.createServer((req, res) => {
  const f = path.join(DIR, req.url === "/" ? "proof5.html" : req.url.split("?")[0]);
...
const userDir = path.join(DIR, ".chrome-profile5");
const chrome = spawn("/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", [
  ...
  `http://localhost:${PORT}/proof5.html`
...
for (let i = 0; i < 100; i++) { if (await ev("window.__ready5===true").catch(() => false)) break; await new Promise((r) => setTimeout(r, 200)); }

const out = {};
out.structuralSharing = JSON.parse(await ev("JSON.stringify(window.structuralSharing([500,2000,4000]))"));
out.changedRangeCost = JSON.parse(await ev("JSON.stringify(window.changedRangeCost([500,2000,4000]))"));
out.serializationCost = JSON.parse(await ev("JSON.stringify(window.serializationCost([500,2000,4000]))"));
out.wordCountCost = JSON.parse(await ev("JSON.stringify(window.wordCountCost([500,2000,4000]))"));
