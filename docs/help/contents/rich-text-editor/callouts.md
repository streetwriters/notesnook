...
This is a [Markdown shortcut](/rich-text-editor/markdown-notes-editing), so it needs the same [Essential plan](/plans-and-limits) as callouts themselves, plus the setting switched on. Markdown shortcuts are **off by default on web and desktop** — turn on `{{mardownShortcuts}}` in `{{settings}}` → `{{customization}}` → `{{editor}}`. On mobile they are on by default. Typing `>` followed by a *space* still makes a plain blockquote, on any plan.
:::
